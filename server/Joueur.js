...
class Joueur {
    ...
    makeMove(keys) {
        ...
        for (let i = 0; i < nbKeys; i++) {
            let keyI = keys[keys.length - i - 1]
                    if (keyI == "ArrowLeft" || keyI == "q") {
                    moveDep[i][0] = -1;
            //direction = "left";
        } else if (keyI == "ArrowRight" || keyI == "d") {
            moveDep[i][0] = 1;
            //direction = "right";
        } else if (keyI == "ArrowUp" || keyI == "z") {
            moveDep[i][1] = -1;
            //direction = "up";
        } else if (keyI == "ArrowDown" || keyI == "s") {
            moveDep[i][1] = 1;
            //direction = "down";
        }
    }
